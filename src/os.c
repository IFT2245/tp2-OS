...
static void* overshadow_thread(void* arg){
    long* r=(long*)arg; long s=0; for(long i=0;i<1000000;i++){ s+=(i%17)+(i%11); }
    *r=s; return NULL;
}

void os_run_hpc_overshadow(void){
    printf("HPC overshadow start\n");
    int n=4; long* vals=(long*)calloc(n,sizeof(long));
    pthread_t* th=(pthread_t*)malloc(n*sizeof(pthread_t));
    for(int i=0;i<n;i++) pthread_create(&th[i],NULL,overshadow_thread,&vals[i]);
