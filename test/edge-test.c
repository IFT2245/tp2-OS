...
TEST(test_extreme_long){
    ...
    if(!pass){
        snprintf(g_test_fail_reason,sizeof(g_test_fail_reason),
                 "Long-burst logs/timeline missing or incomplete.");
        return false;
    }
    return true;
}
...
TEST(test_extreme_short){
    ...
    if(!pass){
        snprintf(g_test_fail_reason,sizeof(g_test_fail_reason),
                 "Short-burst logs/timeline missing or incomplete.");
        return false;
    }
    return true;
}
...
TEST(test_high_load){
    ...
    if(!pass){
        snprintf(g_test_fail_reason,sizeof(g_test_fail_reason),
                 "High load logs/timeline missing or incomplete.");
        return false;
    }
    return true;
}
...
TEST(test_pipeline_edge){
    ...
    if(!pass){
        snprintf(g_test_fail_reason,sizeof(g_test_fail_reason),
                 "Pipeline logs missing or incomplete in edge test.");
        return false;
    }
    return true;
}

static void multi_distrib_test(void){
    os_init();
    for(int i=0;i<3;i++){
...
TEST(test_multi_distrib){
    struct captured_output cap;
    int st=run_function_capture_output(multi_distrib_test,&cap);
    bool pass=(st==0 && strstr(cap.stdout_buf,"Distributed example: fork"));
    if(!pass){
        snprintf(g_test_fail_reason,sizeof(g_test_fail_reason),
                 "Multi distributed logs/timeline missing or incomplete.");
        return false;
    }
    return true;
}
